...
<body>
  ...
  <main>
    ...
    [More work]             → required, `<section class="case-more">` (styles in shell.css): the next two case studies (homepage order, wrapping) as cover + one-liner cards under a centred "More work" heading with rules to both edges; no id, so it stays out of the sidebar. Set `--accent` inline to the page's accent.
  </main>
...
- 25Madison → `#78CE97` (Jain Blue `#C0E0D1` from the brand palette, deepened so it reads as colour against white text)
...
| Body text vs. a directly-adjacent video or image, general | **48px** | `.xx-problem-row`/`.sf-solution-body` (body text ↔ image or video, in a row or stacked), `.xx-solution-inner` (paragraph block ↔ video). **Exception: Design Snapshot's text → video is 34px specifically** (`.xx-cover-img`, or the wrapper around `.xx-cover-frame` on agencyroot/bytedance/salesforce) — see §4. Context↔Impact and Impact↔Design-Snapshot are a separate 64px relationship, also §4 — don't conflate the three. |
| Paragraph → paragraph within the same body block | **24px** | Different relationship from title→body or body→media above — don't conflate. Where paragraphs sit inside a tag+body block whose flex gap is the 16px tag→body value (DiDi's `.didi-section-block`), add `margin-top:8px` on the second paragraph (`.xx-block-text + .xx-block-text`) to land on 24. |
...
| ⚠️ Watch out — card title inside a repeated item | — | The "title → body 36px" rule applies at *every* level, not just the section h2. Bytedance's Direction Exploration item cards (`.bt-explore-text`, e.g. "01: Banner" → its verdict paragraph) are their own title→body relationship and need 36px between the `h3` and the text beneath it, independent of the section-level h2→content gap. Easy to fix the outer one and forget the inner, repeated one. |
| ⚠️ Watch out — fixed image height + wide screenshots | — | Don't give a `.xx-compare-img`-style element (`object-fit:contain`) a fixed `height` when its content is a wide UI screenshot (e.g. ~2:1 aspect ratio). `object-fit:contain` scales to fit width first, so a tall fixed box leaves large empty space above/below the visible picture — inside the image's own box, not in the CSS gap — which reads as "the image is far from the text" even though the actual gap value is correct. Use `height:auto` (or an `aspect-ratio` matching the real image) so the box always matches the picture's own proportions. |
